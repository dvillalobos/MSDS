...
subtitle: CUNY MSDA - IS606 - Lab 0
---
...
![rinterface](/home/mydvtech/CleanGit/MSDS/606/Labs/Lab0/more/rInterface2014.png)
...
source("~/CleanGit/MSDS/606/Labs/Lab0/more/arbuthnot.r")
```
...
```{r view-data-view, eval=TRUE}
#View(present)
present
```

**Answer:** The years included in this data set are from 1940  through 2002.

#### What are the dimensions of the data frame? 
```{r view-data-size, eval=TRUE}
dim(present)
```

**Answer:** The dimensions are 63 rows by 3 columns.

#### what are the variable or column names? 
```{r view-data-names, eval=TRUE}
names(present)
```

**Answer:** The column names are: "year"  "boys"  "girls".

-   How do these counts compare to Arbuthnot's? Are they on a similar scale?

```{r view-data-compare, eval=TRUE}
#View(arbuthnot)
head(present)
head(arbuthnot)
```

**Answer:** The counts are different but the scales are similar.

```{r calc-prop-boys-vars-compare, eval=TRUE}
# Boys
arbuthnotboys <- arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls)
presentboys <-  present$boys / (present$boys + present$girls)

# Girls
arbuthnotgirls <- arbuthnot$girls / (arbuthnot$boys + arbuthnot$girls)
presentgirls <-  present$girls / (present$boys + present$girls)


round(arbuthnotboys - arbuthnotgirls,3)
round(presentboys - presentgirls,3)
```

**Answer:** Yes, they are on a similar scale.
...
```{r plot-boys-to-girl-ratios-by-year, eval=TRUE}
plot(present$year, present$boys / present$girls, type = "l")
presentboys > presentgirls
```

**Answer:** Yes, the proportion is still valid.
...
totalpresent <- cbind(present, total)
presentmax <- which.max(totalpresent$total)
totalpresent[presentmax,]
```

**Answer:** 1961 since it returned the index of the greatest element of present boys + present girls = 4268326.
