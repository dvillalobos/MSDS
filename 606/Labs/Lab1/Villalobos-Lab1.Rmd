...
a bit counter intuitive. As a rule, we omit the column number to see all columns 
...
**Answer:**  From the above graph we can deduct that the difference center is to the left of zero. indicating that the majority of the people think that they are not on their desired weight but overweight.
...
wdiffcdcmen <- data.frame(subset(wdiffcdc, 
                                 wdiffcdc$gender == 'm',  
                                 select = c(wdiff, gender)))
wdiffcdcwomen <- data.frame(subset(wdiffcdc, 
                                   wdiffcdc$gender == 'f',  
                                   select = c(wdiff, gender)))
...
**Answer:**  Based on the above observations and results, we can conclude that men view different their weight than women do.
...
wdiffcdcsdinn <- subset(wdiffcdc, 
                        weight < (wdiffcdcmean + wdiffcdcsd) & 
                        weight > (wdiffcdcmean - wdiffcdcsd))
...
[GitHub](https://github.com/dvillalobos/MSDS/tree/master/606/Labs) | 
