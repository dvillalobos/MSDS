...
This distribution is considered to have a right skew.

```{r}
summary(sim_streak)
```

A typical streak lenght is 0.

The player's longet streak is noted with the Max value in the above table.
...
The streak distribution should be somewhat similar. For that I have run a second simulation in which we can observ the shape of the distribution.
...
Kobe Bryant's distribution of streak lengths is somewhat similar compared to the distribution of streak lengths for the simulated shooter.

```{r}
outcomes <- c("H", "M")
probHKobe <- table(kobe$basket)[1] / sum(table(kobe$basket))
prbH_diff <- data.frame(c())
# Running 100 Simulations
for (i in 1:100)
  {
    sim_basket_i <- sample(outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55))
    probHSim_i <- table(sim_basket_i)[1] / sum(table(sim_basket_i))
    diff <- probHKobe - probHSim_i
    prbH_diff[i,1] <- diff
}
hist(prbH_diff$V1, breaks=100, col="blue", main = "Density Distribution difference after 100 simulations", xlab = "Difference centered at 0")
# Place vertical line
...
#### # A stacked dot plot of dierences from 100 simulations
```{r}
stripchart(prbH_diff$V1, method = "stack", offset = .5, at = .15, pch = 19, main = "Dotplot of Random Values", xlab = "Difference by employing Random Values")
```



Based on what we see, the hot hand model is very similar to Kobe's shooting patterns since the difference in betwen them is minimal and centered to zero.
