...
subtitle: "CUNY MSDA - IS606 - Lab 1"
...
```{r scatter-plot-weight-vs-desired-weight}
#View(cdc)

plot(cdc$weight, cdc$wtdesire)
```

Answer: The relationship seems to be linear or correlated.

Let's consider a new variable: the difference between desired weight (`wtdesire`) and current weight (`weight`). Create this new variable by     subtracting the two columns in the data frame and assigning them to a new 
    object called `wdiff`.
...
What type of data is `wdiff`? If an observation `wdiff` is 0, what does this mean about the person's weight and desired weight. What if `wdiff` is 
    positive or negative?
    
Answer: 

If `wdiff` is 0 then the person currently has his/her desired weight.

If `wdiff` is < 0, then the person know that in order to reach the goal has to loose weight.

If `wdiff` is > 0, then the person know that in order to reach the goal has to gain weight.

Describe the distribution of `wdiff` in terms of its center, shape, and spread, including any plots you use. What does this tell us about how people 
...
barplot(wdiff, main="Difference Weight Distribution",
   xlab= "Individual", ylab="Weight difference")
```

Answer: From the above graphic we can deduct that the difference center is to the left of zero. indicating that the majority of the people think that they are not on their desired weight but overweight.
...
wdiffcdc <- cbind(cdc, wdiff)

wdiffcdcmen <- data.frame(subset(wdiffcdc, wdiffcdc$gender == 'm',  select = c(wdiff, gender)))
wdiffcdcwomen <- data.frame(subset(wdiffcdc, wdiffcdc$gender == 'f',  select = c(wdiff, gender)))

#General Summary for all comabined.
summary(wdiff)
#Summary for Men only
summary(wdiffcdcmen$wdiff)
#Summary for Women only
summary(wdiffcdcwomen$wdiff)

boxplot(wdiffcdc$wdiff ~ wdiffcdc$gender)
```

Answer: Based on the above observations and results, we can conclude that men view different their weight rhan women do.

Now it's time to get creative. Find the mean and standard deviation of `weight` and determine what proportion of the weights are within one 
    standard deviation of the mean.
    
```{r}
# Mean
wdiffcdcmean <- mean(wdiffcdc$weight)
wdiffcdcmean

# Standard deviation
wdiffcdcsd <- sd(wdiffcdc$weight)
wdiffcdcsd

# Create new dataframe with values within range of standard deviation.
wdiffcdcsdinn <- subset(wdiffcdc, weight < (wdiffcdcmean + wdiffcdcsd) & weight > (wdiffcdcmean - wdiffcdcsd))

# Find dimensions
dim(wdiffcdcsdinn)
dim(wdiffcdc)

dim(wdiffcdcsdinn)[1]/dim(wdiffcdc)[1]
```

Answer: The proportion of the weights are within one standard deviation of the mean is 70.76%.
