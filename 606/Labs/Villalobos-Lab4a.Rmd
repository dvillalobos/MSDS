...
`sample_means50` has a total of 5000 elements, these elements conform individual mean calculations for 50 random samples selected on each case.

This sample distribution seems to take a "normal shape" centered around `round(mean(sample_means50),0)`; for this, I can calculate by finding a mean of sample means as follows:

```{r}
mean(sample_means50)
```

If we change the collected sample to 50,000 the distribution will take a better approximation to our original population distribution, that is:
...
For this sample distribution of 50,000 samples seems to take a better approximation of the "normal shape" centered around `round(mean(sample_means50k),0)`; for this, I can calculate by finding a mean of sample means as follows:

```{r}
mean(sample_means50k)
```
...
The resulting vector is composed of 100 means taken from random samples of 50 out of the whole population.

Each element represent the mean for each one of the 50 samples selected at random times on each iteration for 100 times.
...
```{r}
xlimits10 <- range(sample_means10)
xlimits50 <- range(sample_means50)
xlimits100 <- range(sample_means100)

xlimits10[2] - xlimits10[1]
xlimits50[2] - xlimits50[1]
xlimits100[2] - xlimits100[1]
```

As the sample gets larger the center becomes more near to our real population mean and the spread becomes smaller.
...
```{r mean-price50, eval=TRUE}
samp_price <- sample(price, 50)
samp_priceMean <- mean(samp_price)
```   

**Answer:** The best point estimate of the population mean based on the random sample of 50 from `price` is $`r format(samp_priceMean,scientific=FALSE)`.
...
The above calculated population mean is: $`r format(population_mean,scientific=FALSE)`.
...
```{r plot-samps2, eval=TRUE, tidy = FALSE}
par(mfrow = c(1, 2))
...
If we compare the two means histograms, we noticed that as the sample gets bigger the shape of the distribution tend to have a normal distribution centered around a mean of $180,000 with a smaller spread.
...
**Answer:** As the sample of means become bigger, the spread becomes smaller; hence the sample size of 150 means has a smaller spread.
